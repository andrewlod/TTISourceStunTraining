...
@magicWord(category=CATEGORY_CREATIVE, types=[])
def flamengo():
    return dna("toptex", "149")
...
@magicWord(category=CATEGORY_ADMINISTRATOR, types=[str, str, int, int, int])
def smodel(phase, path, x=None, y=None, z=None):
    pos = spellbook.getTarget().getPos()
    if x==None:
        x = int(pos.x)
        y = int(pos.y)
        z = int(pos.z)
    return "func:spawnModel,phase_" + phase + "/models/" + path + "," + str(x) + "," + str(y) + "," + str(z)
